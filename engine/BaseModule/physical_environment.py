...
class GeoGrid:
    ...
    def map_update(self,rain_pos: np.ndarray) -> np.ndarray:
        """
        将矩阵 rain_pos 中的数值加到 map 中对应位置，map 中不为 0 的位置。

        参数:
            map (np.ndarray): 输入的地图矩阵
            rain_pos (np.ndarray): 输入的降水位置矩阵

        返回:
            np.ndarray: 修改后的地图矩阵
        """
        # 确保 map 和 rain_pos 的大小相同
        if self.map.shape != rain_pos.shape:
            raise ValueError("输入的两个矩阵 map 和 rain_pos 的大小必须相同")
        ...
        for i in range(self.map.shape[0]):
            for j in range(self.map.shape[1]):
                if self.map[i, j] != 0:  # 如果 map 中当前位置不为 0
                    self.map[i, j] += rain_pos[i, j]  # 将 rain_pos 中的值加到 map 对应位置
